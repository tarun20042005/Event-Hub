...
import { Calendar, MapPin, Search, Ticket, Users, ArrowRight, Clock, Sparkles, QrCode, Shield } from "lucide-react";
    ...
    <Layout>
        ...
        <div className="relative rounded-3xl overflow-hidden mb-20 shadow-2xl">
          ...
          <div className="relative z-10 p-10 md:p-16 lg:p-20 max-w-2xl">
            ...
            <p className="text-lg text-zinc-300 mb-8 max-w-xl leading-relaxed">
              Browse thousands of events, book instant tickets, and get your digital QR pass. All in one place.
            </p>
            <div className="flex flex-col sm:flex-row gap-4">
              <Link href="/register">
                <Button size="lg" className="w-full sm:w-auto text-lg px-8 shadow-lg shadow-primary/30">
                  Start Exploring
          ...
          {[
            {
              ...
              description: "Get your digital ticket instantly with a scannable QR code"
            },
            {
              icon: Sparkles,
              title: "Easy Booking",
              description: "Browse, search, and book events in just a few clicks"
            },
            {
              icon: Shield,
              title: "Secure & Trusted",
              description: "Safe payment collection at venue with verified organizers"
            }
          ].map((feature, i) => (
            <motion.div
              key={i}
              ...
              className="rounded-2xl p-8 bg-gradient-to-br from-primary/5 to-accent/5 border border-border/50 hover:border-primary/30 transition-colors"
            >
      ...
      <div className="mb-16">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-8 mb-12">
          <div>
            <p className="text-sm font-semibold text-primary mb-2 uppercase tracking-wide">Browse Events</p>
            <h2 className="text-4xl font-bold tracking-tight mb-3">Upcoming Events</h2>
            <p className="text-muted-foreground text-base max-w-xl">
              Discover amazing events happening near you. Search, filter, and book your tickets instantly.
            </p>
          </div>
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2, 3, 4, 5, 6].map(i => (
              <div key={i} className="bg-card rounded-3xl h-96 animate-pulse border border-border/50" />
            ))}
          </div>
        ) : events?.length === 0 ? (
          <div className="text-center py-24 rounded-3xl border-2 border-dashed border-border">
            ...
            <p className="text-muted-foreground mb-8 max-w-sm mx-auto">
              {search || date ? 'No events match your filters. Try adjusting your search.' : 'Check back soon for amazing events!'}
            </p>
            {(search || date) && (
      ...
      {/* CTA Section */}
      {!user && (
        <div className="mt-20 rounded-3xl overflow-hidden bg-gradient-to-r from-primary/20 to-accent/20 border border-primary/30 p-12 md:p-16 text-center">
          <h2 className="text-3xl md:text-4xl font-bold mb-4">Ready to Book Your First Event?</h2>
          <p className="text-muted-foreground text-lg mb-8 max-w-2xl mx-auto">
            Join thousands of attendees discovering and booking amazing events. Create your account now!
          </p>
          <Link href="/register">
